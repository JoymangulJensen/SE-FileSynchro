...
/**
 *Envoie des dossier vers le client
 */
class EnvoieDossier implements Runnable {
    ...
    public EnvoieDossier(ObjectOutputStream out, String src, String dest) {
        oos = out;
        source = src;
        destination = dest;
            ...
            for (WatchEvent<?> event : watchKey.pollEvents()) {
                ...
                if (event.kind() == ENTRY_MODIFY) {
                    ...
                    if (new File(srcMod).isFile()) {
                        envoieFichier(srcMod, destination + relative);  //envoire le fichier
                    }
                    watchKey.reset();
                }
            }
    ...
    public void envoieFichier(String source, String destination) {
        ...
        if (taileDos < max) {   //verification si le quota n'as pas été dépasée
            ...
            System.out.println("quota depassé");
        }
